...
public class DataAccess20181003 {
	Logger logger = Logger.getLogger("DataAccess");
	Rutinas mylib = new Rutinas();
	GlobalParams gParams;
	MysqlAPI dbConn;
	
	public DataAccess20181003(GlobalParams m) {
		gParams = m;
					...
					case "BTB":
						...
						if (dbConn.executeQuery(vSql)) {
							...
							if (rs.next()) {
								...
								logger.info(logmsg+"Serializando respuesta en objeto clase BackTable()");
								btb = (BackTable) mylib.serializeJSonStringToObject(resp, BackTable.class);
								
								logger.info(logmsg+"Se ha serializado correctamente el objeto clase BackTable()");
